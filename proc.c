...
void MLFQ_scheduler(struct cpu *c)
{
  ...
  int exceededTimedur = 25 * ticksTodur;
  int i, j;

  for (int t = 1; t < 5; t++)
  {
    for (int qi = 0; qi <= cxj[t]; qi++)
    {
      struct proc *pp = qxj[t][qi];
      if (pp == 0) continue;
      if (pp->state != RUNNABLE) continue;

      int pid = pp->pid;
      if (pstat_var.current_queue[pid] != t)
        continue;

      if (ticks - pstat_var.runtime[pid] >= exceededTimedur)
      {
        for (j = qi; j <= cxj[t] - 1; j++)
          qxj[t][j] = qxj[t][j + 1];
        qxj[t][cxj[t]] = 0;
        cxj[t]--;

        cxj[t - 1]++;
        pstat_var.current_queue[pid] = t - 1;
        pstat_var.runtime[pid] = ticks;
        qxj[t - 1][cxj[t - 1]] = pp;
        pstat_var.num_run[pid] = 0;
